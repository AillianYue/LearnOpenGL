...
namespace render {
Mesh2D::Mesh2D(const std::string &path, const std::vector<float> &vert) {
    m_vert = vert;
    if(!InitOpenGl()) {
...
bool Mesh2D::Draw(std::shared_ptr<opengl::Shader> shader) {
    shader->Use();
